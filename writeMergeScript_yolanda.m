...
filename=[ 'Yolanda' dateStr '.txt'];
fid=fopen(filename,'wt');
fprintf(fid,'Set 1:');
f2load = [path_name 'y' dateStr '\info\y' dateStr '_param.mat'];
...
load([path_name 'y' dateStr '\info\y' dateStr '_param.mat']);
for i=1:length(SESSparam.SubSess), 
    line=sprintf('\nE%s y%02d%02d %d..%d ',dateStr,DDFparam.ID,i, SESSparam.SubSess(i).Files(1),SESSparam.SubSess(i).Files(2));
